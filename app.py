# Abstract
                ...
                article = {
                    ...
                    'citations': 0,  # CORE não fornece contagem de citações
                    'url': url,
                    'abstract': abstract,
                    'venue': journal
                }
...
# --- FUNÇÃO PRINCIPAL DE BUSCA EXPANDIDA ---
...
def search_all_sources(query, min_year, min_citations, selected_sources=None):
    ...
    # Executa buscas em paralelo com limite de workers
...
def deduplicate_articles(articles, saved_articles_ids=None):
    ...
    for article in articles:
        ...
        # Verifica duplicatas por título (similaridade)
        is_duplicate = False
        ...
        # Verifica duplicatas por URL ou ID
...
def handle_search():
    ...
    try:
        ...
        selected_sources = data.get('sources', None)  # Novo parâmetro opcional
        ...
        # Executa buscas para cada estratégia
        all_found_articles = []
        ...
        # Ordena por relevância (citações * fator de ano)
        current_year = datetime.datetime.now().year
        for article in unique_articles:
            year = article.get('year', current_year)
            citations = article.get('citations', 0)
            age = current_year - year
            recency_factor = max(0.5, 1.0 - (age * 0.05))  # Decai 5% por ano
...
def handle_get_sources():
    ...
    sources = {
        ...
        'web_of_science': {
            ...
            'status': 'active' if WOS_API_KEY and WOS_API_KEY != 'sua_chave_aqui' else 'needs_key',
            'requires_key': True
        },
        'doaj': {
            'name': 'DOAJ',
...
# Manter todas as outras rotas originais...
@app.route('/api/import-bib', methods=['POST'])
def handle_bib_import():
    """Rota para importação de arquivos BibTeX - MANTIDA"""
    try:
...
# Manter todas as outras rotas originais (add-by-url, generate, manage/load, manage/update, build-framework)...

@app.route('/api/add-by-url', methods=['POST'])
def handle_add_by_url():
    """Rota para adicionar artigo por URL - MANTIDA"""
    try:
...
def handle_generate():
    """Rota para gerar fichamentos - MANTIDA"""
    try:
...
def handle_load_saved():
    """Rota para carregar artigos salvos - MANTIDA"""
    try:
...
def handle_update_saved():
    """Rota para atualizar artigos salvos - MANTIDA"""
    try:
...
def handle_build_framework():
    """Rota para construir referencial teórico - MANTIDA"""
    try:
...
def scrape_researchgate_metadata(url):
    """Extrai metadados do ResearchGate - MANTIDA"""
    try:
...
def health_check():
    ...
    status = {
        ...
        "apis": {
            ...
            "web_of_science": "configured" if WOS_API_KEY and WOS_API_KEY != 'sua_chave_aqui' else "missing",
    ...
    # Adiciona fontes que requerem chaves
    if WOS_API_KEY and WOS_API_KEY != 'sua_chave_aqui':
...
if __name__ == '__main__':
    ...
    log_info(f"Web of Science API: {'Sim' if WOS_API_KEY and WOS_API_KEY != 'sua_chave_aqui' else 'Não'}")
    log_info(f"CORE API: {'Sim' if CORE_API_KEY else 'Não'}")
    
    app.run(host='0.0.0.0', port=port, debug=debug)# --- APLICAÇÃO PARA RENDER COM MÚLTIPLAS FONTES ACADÊMICAS ---
# Versão expandida com Web of Science, DOAJ, arXiv, PubMed e outras fontes
...
WOS_API_KEY = os.environ.get('WOS_API_KEY', '90a06eadc36671085f013c74e70a3daab9b788c0')
CORE_API_KEY = os.environ.get('CORE_API_KEY')  # https://core.ac.uk/api-keys/register
...
# --- FUNÇÕES DE BUSCA ACADÊMICA EXPANDIDAS ---

def search_semantic_scholar(query, min_year, min_citations):
    """Busca no Semantic Scholar - MANTIDO ORIGINAL"""
    try:
...
def search_crossref(query, min_year):
    """Busca no CrossRef - MANTIDO ORIGINAL"""
    try:
...
def search_web_of_science(query, min_year, min_citations):
    ...
    if not WOS_API_KEY or WOS_API_KEY == 'sua_chave_aqui':
        ...
        for endpoint in endpoints:
            try:
                ...
                if response.status_code == 200:
                    ...
                    log_error(f"Web of Science: HTTP {response.status_code} - {response.text}")
                    continue
...
def search_doaj(query, min_year):
    ...
    try:
        ...
        # API do DOAJ é gratuita e não requer chave
        ...
        for item in results:
            try:
                ...
                if not year:
                    # Tenta extrair do campo de data
                ...
                article = {
                    ...
                    'citations': 0,  # DOAJ não fornece contagem de citações
                    'url': url,
                    'abstract': abstract,
                    'venue': journal_title
                }
...
def search_arxiv(query, min_year):
    ...
    try:
        ...
        # API do arXiv usando Atom feeds
        ...
        for entry in entries:
            try:
                ...
                article = {
                    ...
                    'citations': 0,  # arXiv não fornece contagem de citações
                    'url': url,
                    'abstract': abstract,
...
def search_openalex(query, min_year, min_citations):
    """Busca no OpenAlex (substituto gratuito do Microsoft Academic)"""
    try:
        ...
        for item in results:
            try:
                ...
                # Filtros já aplicados na API, mas verificamos novamente
                if year < min_year or citations < min_citations:
                    continue
                ...
                # Se há índice invertido do abstract, tenta reconstruir (simplificado)
                if abstract_url and isinstance(abstract_url, dict):
                    try:
                        ...
                        if len(abstract) > 500:  # Limita tamanho
        ...
        for pubmed_article in root.findall('.//PubmedArticle'):
            try:
                ...
                article = {
                    ...
                    'citations': 0,  # PubMed não fornece contagem direta de citações
                    'url': url,
                    'abstract': abstract,
                    'venue': journal
                }
...
def search_core(query, min_year):
    """Busca no CORE (requer API key gratuita)"""
    if not CORE_API_KEY:
