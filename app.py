...
def scrape_researchgate_metadata(url):
    try:
        ...
        response = requests.get(url, headers=headers, timeout=10); response.raise_for_status(); soup = BeautifulSoup(response.content, 'html.parser')
        title = soup.find('h1').text.strip() if soup.find('h1') else 'Título não encontrado'
        authors_div = soup.find('div', class_='research-detail-header-section__authors'); authors = [a.text for a in authors_div.find_all('a')] if authors_div else []
        abstract_div = soup.find('div', class_='research-detail-middle-section__abstract'); abstract = abstract_div.find('div').text.strip() if abstract_div else 'Resumo não encontrado.'
        date_div = soup.find('div', string='Date of Publication'); year = int(date_div.find_next_sibling('div').text.split(', ')[-1]) if date_div and date_div.find_next_sibling('div') else datetime.datetime.now().year
        return {'id': "rg-" + url.split('/')[-1], 'title': title, 'authors': authors, 'year': year, 'source': 'ResearchGate (Manual)', 'citations': 0, 'url': url, 'abstract': abstract, 'topic': 'Adicionado Manualmente'}
    except Exception as e: print(f"Erro ao ler URL do ResearchGate: {e}"); return None
def deduplicate_articles(articles, saved_articles_ids=None):
    ...
    for article in articles:
        ...
        if identifier and identifier not in seen: unique_articles.append(article); seen.add(identifier)
...
def handle_bib_import():
    ...
    try:
        content = file.read().decode('utf-8'); bib_database = bibtexparser.loads(content); articles = []
        for entry in bib_database.entries:
            authors = [name.strip() for name in entry.get('author', '').split(' and ')]
            articles.append({'id': entry.get('doi') or entry.get('ID'), 'title': entry.get('title', 'N/A').replace('{', '').replace('}', ''), 'authors': authors, 'year': int(entry.get('year', 0)), 'source': f"Importado ({entry.get('journal', 'N/A')})", 'citations': 0, 'url': entry.get('url') or f"https://doi.org/{entry.get('doi')}" if entry.get('doi') else '#', 'abstract': entry.get('abstract', 'Resumo não disponível.'), 'topic': 'Importado'})
        service = get_drive_service(); folder_id = get_or_create_folder(service, DRIVE_FOLDER_NAME)
        saved_articles = load_saved_articles_from_drive(service, folder_id); saved_ids = {a['id'] for a in saved_articles}
        return jsonify(deduplicate_articles(articles, saved_ids))
    except Exception as e: print(f"Erro ao processar ficheiro BibTeX: {e}"); return jsonify({"error": "O ficheiro enviado não é um BibTeX válido."}), 500

@app.route('/api/add-by-url', methods=['POST'])
def handle_add_by_url():
    url = request.json.get('url')
    if not url: return jsonify({"error": "URL não fornecida."}), 400
    if "researchgate.net" not in url: return jsonify({"error": "Atualmente, apenas links do ResearchGate são suportados."}), 400
    ...
    try:
        ...
        if article_data['id'] in saved_ids: return jsonify({"status": "info", "message": "Este artigo já existe no seu fichamento."})
    ...
    except Exception as e: print(f"Erro em /api/add-by-url: {e}"); return jsonify({"status": "error", "message": str(e)}), 500
